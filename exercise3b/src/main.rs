...
fn main() {
    ...
    let distance: u32 = shortest_wires_to_intersection(wire_a_coords, wire_b_coords);
...
fn shortest_wires_to_intersection(coords_a: Vec<[i32; 2]>, coords_b: Vec<[i32; 2]>) -> u32 {
    let mut distance: u32 = 0;
    let mut length_a: u32 = 0;
    let mut length_b: u32 = 0;

    for point_a in &coords_a {
        for point_b in &coords_b {
            if point_a == point_b {
                let this_distance = length_a + length_b;

                if length_a + length_b < distance || distance == 0 {
...
mod tests {
    ...
    fn find_distance_from_vectors_test() {
        ...
        let distance = shortest_wires_to_intersection(wire_a_coords, wire_b_coords);
